...
- [x] 跑通 `kernels/elementwise`
  - [x] 读：`kernels/elementwise/README.md`
  - [x] 跑：`python3 kernels/elementwise/elementwise.py`（或进入目录执行）

## 你要看懂的点（够用即可）

- [x] grid/block/thread 的映射方式
- [x] 一维 index 访问模式是否 coalesced（同一 warp 访问是否连续）
- [x] Python bindings 大致做了什么（加载 extension、调用 kernel、对齐 PyTorch 输出）

## 本周产出（必做）

- [x] 一次运行日志：记录 1–2 组 shape 的时间对比（自实现 vs PyTorch）
- [x] 一句话结论：瓶颈更像“算力”还是“带宽”（先凭直觉即可，下周用 `ncu` 验证）
